...
// STORY-150 — Voice picked by name, not typed by id (Epic Y / SPEC F54, closes gitea-#224).
//
...
//
// The Station:Persona:ActiveId scenario this file used to own (gitea-#225) is gone along with
// the key itself (SPEC F91.5, PLAN T120/T127 — the format clock is the only thing that names an
// on-air persona now); roster-page.spec.tsx's "no persona activation control on the Settings
// page" test is this file's replacement coverage.
  ...
  describe("Scenario: submission plumbing is untouched", () => {
    it("a dropdown-picked change rides the same changed-keys PUT batch (F54.4)", async () => {
      ...
      const settings = [
        ...
        settingDto({ key: "Loudness:TargetLufs", value: "-16", source: "default" as const, applyMode: "live" as const, kind: "number" as const, unit: "LUFS" }),
      ];
