 ...
 * unbounded free-form prose the moment they arrive off the wire — mirrors `FontLibraryPackDto`'s own
 * documented "unbounded free-form prose" posture for `family`/`style` (`font-format.ts`'s own sibling
 * remarks), but with no server-side length gate of its own anywhere in `AvatarPackController` (unlike
 * an item's `name` at INSTALL time, bounded to 64 chars by `IsValidItemName` — a bound this SHELF/
 * DETAIL read, and the browsing-before-install face grid in particular, never benefits from:
 ...
 *
 * React already escapes every character this renders — this clamp is layout protection only, never a
 * security boundary. `MAX_DISPLAY_LENGTH` is a plain, generous ceiling for a one-line card title/
 * label, not a value SPEC F128 pins anywhere.
 */
