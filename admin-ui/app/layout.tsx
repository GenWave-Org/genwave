...
// Reads the genwave-mode cookie during the server render and stamps data-theme
// on <html> before any HTML reaches the browser — first paint never flashes the
// wrong mode (SPEC F28.4). A garbage or absent cookie value parses to null, so
...
// (data-theme is the DOM attribute name CSS already keys on for light/dark — it
// predates and is independent of the genwave-theme PALETTE cookie, PLAN T164.)
export default async function RootLayout({
  children,
}: RootLayoutProps): Promise<ReactNode> {
  const cookieStore = await cookies();
  const mode = parseMode(cookieStore.get(MODE_COOKIE_NAME)?.value);

  return (
    <html lang="en" data-theme={mode ?? undefined}>
      <body>
        {/*
