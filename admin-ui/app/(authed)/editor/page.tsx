...
import type { ThemeSummaryDto, VendoredFontDto } from "./types";
 ...
 * means nothing gets offered. This page's own two GET routes (`/api/themes`, `/api/fonts/vendored`)
...
export default async function EditorPage(): Promise<ReactNode> {
  ...
  const [themes, vendoredFaces] = await Promise.all([
    fetchListOrEmpty<ThemeSummaryDto>("/api/themes", cookieHeader),
    fetchListOrEmpty<VendoredFontDto>("/api/fonts/vendored", cookieHeader),
  ]);
  ...
  return (
    <main>
      ...
      <div className="mt-4">
        <EditorClient themes={themes} vendoredFaces={vendoredFaces} />
      </div>
    </main>
  );
}
