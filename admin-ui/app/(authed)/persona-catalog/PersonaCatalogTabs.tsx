...
/**
 * Personas | Themes | Fonts | Shows tab strip for the Community Catalog shelf (gh-#372) —
 * URL-driven via `?kind=`, the shared `TabStrip` markup (gh-#393's extraction). One tab per kind:
 * the flat mixed grid gave no way to tell a persona card from a show card without opening it
 * (the issue's own complaint — neither kind carries a badge), and the pile got worse with every
 * kind the shelf gained. Every tab renders even when its kind has nothing on the shelf (the
 * gh-#393 ruling applied here too) — the tab's own empty state names the kind instead.
 */
