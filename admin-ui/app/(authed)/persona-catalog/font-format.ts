 ...
 * font wire (`FontLibraryPackDto.license`/`version`/`subset`): different DTOs, same three field
 ...
 * mirrors the retired Library/Wardrobe page's own pre-existing line) — omitting whichever of the
 * three is absent (`version` is genuinely optional even on a cleanly-parsed manifest; `license`/
 * `subset` are `null` only when the manifest failed to parse, degrade, or — on the catalog wire —
 * the entry is unreachable). Degrades to "Licence unknown" rather than an empty line on the
 * all-null edge — a reviewer never sees a blank fact where the panel's whole point is showing this
 * one. Used by `FontDetailPanel` (pre-install review); the retired Wardrobe page's own pack cards
 * shared this exact wording too. */
