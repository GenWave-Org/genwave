...
import { updateAdSpot, type AdSpotDto, type AdSpotSaveBody, type AdVoicePlanEntry } from "@/lib/ads-api";
...
export interface AdSpotEditorProps {
  /** The spot being edited (SPEC F162.1's "edit drafts") — the SpotWizard (PLAN T448) owns creating
   * a new one; this component only ever edits an already-committed row. */
  initial: AdSpotDto;
 ...
 *
 * Edits an existing spot only (`PATCH /api/ads/{id}`, If-Match from `initial.version`) — creating
 * one is the SpotWizard's own job (PLAN T448 ruling).
 *
 ...
 * looking like it cleared the value. Three fields carry that risk, and each is closed a different
 * way, chosen by what the control itself can express:
 * <list type="bullet">
 ...
 * </list>
 */
export function AdSpotEditor({ initial, sponsors, onSaved, onCancel }: AdSpotEditorProps): ReactNode {
  const restoreFocus = useRestoreFocus("on-mount");

  const [sponsorId, setSponsorId] = useState<number | null>(initial.sponsorId);
  const [title, setTitle] = useState(initial.title);
  const [brief, setBrief] = useState(initial.brief ?? "");
  const [script, setScript] = useState(initial.script ?? "");
  const [spotSeconds, setSpotSeconds] = useState<number>(initial.spotSeconds);
  const [voicesByTag, setVoicesByTag] = useState<Record<string, string>>(() =>
    initialVoicesByTag(initial.voicePlan)
  );
  const [bed, setBed] = useState<BedCandidate | null>(initialBedCandidate(initial.bedMediaId));
  ...
  const pinnedTags = useMemo(() => new Set(initial.voicePlan?.map((entry) => entry.tag) ?? []), [initial]);

  // F1 — a bed clear is only ever honest when there is nothing already committed to silently fail
  // to clear. See the class remarks' "Bed" bullet.
  const canClearBed = initial.bedMediaId === null;
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (initial.brief !== null && brief.trim() === "") {
      setStatus({
        kind: "error",
    ...
    if (initial.script !== null && script.trim() === "") {
      setStatus({
        kind: "error",
    ...
    const outcome = await updateAdSpot(initial.id, initial.version, body);
  ...
  return (
    ...
    >
      <Dialog.Portal>
        ...
        >
          <Dialog.Title className="font-display text-[1.1rem] text-ink">Edit spot</Dialog.Title>
