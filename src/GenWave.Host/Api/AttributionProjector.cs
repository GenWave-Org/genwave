...
/// <summary>
/// Attribution projection (SPEC F169.1/F169.2, STORY-404, PLAN T419) — iterates every installed
/// jingle-pack/font-pack/voice-pack row and projects its credits for the About page epic (gh-#16).
/// Shared by <see cref="AttributionsController"/> (<c>GET /api/attributions</c>) and
/// <see cref="AboutController"/> (<c>GET /api/about</c>'s <c>attributions</c> field, SPEC F207.2,
/// STORY-474, PLAN T561) — one projection, injected into both, so the two responses can never drift
/// apart into two independently-maintained copies of the same rule. There is deliberately no
/// separate <c>station.attribution</c> table: every credit line here is re-derived, on every request,
/// from the pack's own stored <c>definition</c> jsonb (db/45's own header remarks) — a mirror table
/// would drift the moment a pack's manifest and its own attribution copy disagreed.
///
/// <para>
...
/// </para>
/// </summary>
public sealed class AttributionProjector(
    IJinglePackStore jinglePackStore,
    IFontPackStore fontPackStore,
    IVoicePackStore voicePackStore,
    ILogger<AttributionProjector> logger)
{
    const string SyntheticBlendLicense = "Synthetic blend";
    const string Cc0License = "CC0";
    const string CcByLicense = "CC-BY";

    /// <summary>Builds the SAME <see cref="AttributionsResponse"/> both <c>GET /api/attributions</c>
    /// and <c>GET /api/about</c> serve — see this type's own remarks for the full projection
    /// contract.</summary>
    public async Task<AttributionsResponse> BuildAsync(CancellationToken ct)
    {
        ...
        return new AttributionsResponse(groups);
    }
