    ...
    IArtworkUrlEchoValidator? artworkUrlEchoValidator = null)
{
    ...
    // Nudge (SPEC F151.1/F151.2, STORY-371, PLAN T370) rides the SAME two paths one member further
    // still: a feeder push captures the MediaItem's own Nudge (null unless the pick reached the
    // persona ranker's rung 0); an engine-initiated advance always carries null — safe rotation never
    // reaches the ranker either. TrackAired forwards it verbatim to the booth log's own pick jsonb
    // stamp, same as RotationRelax.
    ...
    /// DurationMs/PersonaPick/ArtworkUrl/DjName/SegmentKind/CrosstalkScript/RotationRelax/Nudge all
    /// carry the SAME remarks the field declaration above documents.
    /// </summary>
    sealed record PushedItemMeta(
        ...
        int? RotationRelax, double? Nudge);
            ...
            if (onAirIsReal)
            {
                ...
                if (!feederOwnedIds.Contains(mediaId))
                {
                    ...
                    pushedMeta[mediaId] = new PushedItemMeta(
                        ...
                        DjName: null, SegmentKind: null, CrosstalkScript: null, RotationRelax: null, Nudge: null);
                }
                else
                {
        ...
        if (prepared == 0)                            // (re)fill the one-ahead chain
        {
            ...
            for (var i = 0; i < MaxChainLength; i++)
            {
                ...
                pushedMeta[item.MediaId] = new PushedItemMeta(
                    ...
                    item.DjName, item.SegmentKind, item.CrosstalkScript, item.RotationRelax, item.Nudge);
