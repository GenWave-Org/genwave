...
/// <summary>
/// The claim classes <see cref="CopyClaims"/> can report a violation for (SPEC F138.1, F138.3,
/// F144.3): the three F138.1 extracted-claim classes (<see cref="DigitRun"/>, <see cref="Weekday"/>,
...
///
/// <see cref="AnnouncementCore"/> (SPEC F144.3, STORY-358, PLAN T342) is a fifth class with the
/// OPPOSITE shape from the other four: those report a claim the copy ADDED with nothing to support
/// it; this one reports the owner's own message text MISSING from the copy — see
/// <see cref="CopyClaims.CheckContainment"/>'s own remarks. <see cref="CopyClaims.CheckClock"/> never
/// produces it either.
/// </summary>
public enum ClaimClass
{
    ...
    /// <summary>
    /// The owner's own announcement message did not survive, case-folded, in the rendered copy (SPEC
    /// F144.3, PLAN T342). <see cref="CopyClaims.CheckContainment"/> only.
    /// </summary>
    AnnouncementCore,
}
