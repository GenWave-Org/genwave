...
Since v5.5.0 the library tends itself. The **Gardener** runs five rot passes on a timer — dead files, near-duplicates, stale metadata, shelf dust (unaired past its days), and rows no schedule envelope admits — surfacing findings on a **Gardener page** an operator works through and dismisses, not a silent auto-fixer. Listeners can thumb the current track up or down from the spectator page (opt-in, off by default) alongside the existing operator thumbs; both feed one decayed **rotation nudge** into persona selection, so what actually gets played shapes what plays next. **Deep Cuts** shows can pin a rotation predicate that favors the under-played, with a relax ladder so a thin catalog never starves the boundary. File actions — retag, rename, move — are opt-in and jailed to `MediaRoot`, the one Gardener capability that writes to your media at all; see [DEPLOYMENT.md](DEPLOYMENT.md).
...
│  └─ 02..40-*-migration.sh # idempotent in-place upgrades, one per shipped feature —
...
rings them) and the v5.4.1 riders (the gate learns the station's name; the now-playing
sensor works on a shipped appliance with the admin plane off) — lives in
