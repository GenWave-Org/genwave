...
[HARDWARE.md](HARDWARE.md) for what GenWave runs on, how to size a box, and (its "LLM model
floor" section) which DJ model to run — every model benched from 3B to 12B passed the truth
gate, so size the model to your box, not to your trust.
...
./setup.sh          # the first-run wizard (since v5.3.0): four questions — prebuilt images or build
                    # from source, where the music lives, which topology, admin on/off — then
                    # it generates every secret, writes .env atomically, and launches
```

Re-running `./setup.sh` on a box that already has a `.env` never rewrites it: it verifies the
existing install against the machine (drift report), and `--repair` fixes what it can. The same
wizard is what [download.genwaveradio.com](https://download.genwaveradio.com/) fetches.

Or, by hand — the dev flow the wizard wraps:
...
Since v5.5.0 the library tends itself. The **Gardener** runs five rot passes on a timer — dead files, near-duplicates, stale metadata, shelf dust (unaired past its days), and rows no schedule envelope admits — surfacing findings on a **Gardener page** an operator works through and dismisses, not a silent auto-fixer; since v5.5.1 the queue is worked through five badged kind tabs with server-side paging that never splits a duplicate cluster. A **Gardener tile** and a **rotation-health block** (playable / never-aired / aired-once counts) sit on the dashboard. Listeners can thumb the current track up or down from the spectator page (`Station:Thumbs:Enabled` — live, off by default) alongside the existing operator thumbs; both feed one decayed **rotation nudge** into persona selection, so what actually gets played shapes what plays next. **Deep Cuts** shows can pin a rotation predicate that favors the under-played, with a relax ladder so a thin catalog never starves the boundary. File actions — retag, rename, move, never delete — are the one Gardener capability that writes to your media at all: jailed to `MediaRoot`, a `.gwbak` backup behind every retag, and armed only when *both* knobs agree (`Gardener:FileActions:Enabled` **and** the `compose.fileactions.yaml` overlay that widens the api's media mount to `:rw`); see [DEPLOYMENT.md](DEPLOYMENT.md).

Since v5.6.0 the station opens two doors. Third-party code can extend it in-process through the **plugin door** — a narrow SPI (`IContextProvider`/`IAdSpotSource`, additive-only, whole-plugin skip on any failure, never a partial load) opt-in behind two independent env knobs; see [PLUGINS.md](PLUGINS.md) for the licensing posture and [`examples/genwave-plugin-example/`](examples/genwave-plugin-example/) for a real, loadable reference plugin. And the station authors its own commercial breaks: **ads** are drafted by the LLM against operator-authored briefs, validated (duration fit, blocklisted names, no real phone numbers), rendered once ahead of air through the same TTS/loudness pipeline as everything else, and slotted into rotation on a live cadence (`Station:Ads:EveryNUnits`, `0` = off) — structurally fenced out of music rotation, requests, and `/media/random` the same way every other authored imaging segment is. The Admin UI's **Ads page** is where spots are drafted, approved, retried, and retired; a brief corpus can also arrive pre-packaged via the Community Catalog's new **ad-pack** kind, installed one-click like any other pack.

Since v5.7.0 the shelf carries sound, not just words. **Voice packs** install extra Kokoro voices into a shared volume the synthesizer rescans per request, so a new voice is speakable on the next line with no restart; **jingle packs** land background-music loops, hits, and signature clips in the library through the same loudness and cue enrichment a scanned track gets, with structured attribution (CC0/CC-BY only) served at `GET /api/attributions`. Ads use both: every spot gets an announcer plus two cast voices, and renders over installed background music ducked under the voices. Since v5.8.0 every ad has a customer: **sponsors** own the briefs, a paused sponsor's spots leave rotation on the next pick, and "New spot…" is a five-step guided flow (sponsor, angle and length, script, hear it, approve as heard) with a bounded job queue behind the "Write it for me" button. Shows can carry a sponsor too.

### Resilience & operator tools

The broadcast never depends on a sick dependency. **LLM failure is a mode, not an error**: consecutive failures walk the station Normal → Soft (one real LLM attempt per cooldown window, template copy otherwise) → Hard (zero LLM calls); background health probes plus a cooldown walk it back up, and an operator can pin any mode live (`Llm:DegradationPin`). **The DJ's patter is checked before it airs** (v5.4.0's truth gate): each LLM-authored lead-in, back-announce, time check, or context segment is checked against the fact block it was given and the station clock. The checks are literal, not semantic: numbers must appear in the facts (a rounded "21" against a fact of "20.6" is rejected), weekday names and weather condition words must match, and clock words must fit the hour ("tonight" at noon is rejected). A fluent sentence with no number, weekday, or weather word in it is not a claim and passes; when a match is uncertain the gate passes rather than rejects. A rejected line is re-asked once; a second rejection falls back to the template line for lead-ins and back-announces, and drops the segment for everything else. Ad scripts and crosstalk have their own narrower checks (duration fit, blocklisted names, phone numbers) and are skipped, not re-asked. **A DJ never speaks in someone else's voice** (the v5.1.0 voice contract): if a DJ's own voice can't be produced, the break simply doesn't air — the music continues uninterrupted, and the Voice tile on the health surface tells you the engine is down versus the DJ having nothing to say. Voice **failover is opt-in**: enable the `fallback` compose profile to run the Piper sidecar and set `Tts:Fallback:Endpoint` (live, no restart) to accept a substitute voice instead of silence — the previous always-on behavior, now a choice ([DEPLOYMENT.md](DEPLOYMENT.md) has both halves). Every spoken line passes one normalization chokepoint (reasoning-block scrub, markdown strip, operator **pronunciation corrections** — editable with live preview under Settings → TTS, e.g. `MacLeod → Muh-cloud`). The **Booth log** page answers "what did the DJ do and say at 9:14" as a persistent narrative feed (track starts, patter, mode changes, 14-day retention), with an **LLM call inspector** tab showing the last ~50 calls (prompt, response, persona, timing, mode, and a typed cause — Timeout / OverLength / TruthGateReject / EmptyCompletion / … — in-memory, never persisted); a red LLM tile names its dominant cause and model instead of just being red. The **Health** page is a container-level view of the running stack — state, CPU, memory per service — fed by a read-only, endpoint-allowlisted docker-stats sidecar (the API never touches the docker socket). **Station Imaging** (the operator-authored always-airable segments: liners, station IDs, jingles, promos) is authored in the Admin UI and rendered through the same TTS + loudness pipeline as everything else. Dependency health probes **debounce** — a verdict flips unhealthy only after consecutive probe failures (a slow TTS render is not an outage), with interval, timeout, and threshold all live-editable settings. MusicBrainz lookups are throttled to 1 req/s with a version-stamped User-Agent, and misses are stamped so they're never re-asked.
...
├─ compose.pinned.yaml     # published-image pins (`--pinned`); compose.piper-only.yaml, compose.demo.yaml,
│                          #   compose.fileactions.yaml, compose.plugins.yaml are the other overlays
├─ .env.example            # secrets template → copy to .env
├─ setup.sh                # first-run wizard; launch.sh (start), build.sh (build + test + images), migrate.sh
├─ engine/
│  └─ genwave.liq          # Liquidsoap playout script
├─ db/
│  ├─ 01-library.sh        # library schema + library_svc role (canonical fresh install)
│  └─ NN-*-migration.sh    # idempotent in-place upgrades, one per shipped feature —
...
│  ├─ soak-check.sh              # the soak-runbook checkpoint script (HARDWARE.md)
│  ├─ demo_hour_gate.sql         # the demo-hour acceptance query (booth log, one hour of air)
│  ├─ IconPackAuthor/            # authoring tool for catalog icon packs (v5.2.0)
...
└─ src/                    # C# solution (.NET 10)
   ...
   ├─ GenWave.Ads/           #   ad briefs, sponsors, script validation, the AdSpotWorker
   ├─ GenWave.Plugins/       #   the plugin door: SPI load context, whole-plugin skip on failure
...
└─ tests/                  # nine xUnit projects, one per src project plus Architecture.Tests
```

## Tests

```bash
# PR tier — what CI runs (no Docker; ffmpeg on PATH, the artwork specs drive the real binary):
dotnet test GenWave.sln --filter "Category!=Integration"

# Full suite including library + Kokoro integration tests (Docker + ffmpeg):
dotnet test GenWave.sln

# The Host suite alone on a small box — cap xUnit's parallelism or docker-backed facts time out:
dotnet test tests/GenWave.Host.Tests -- xUnit.MaxParallelThreads=3
...
# lint, production build, stylesheet link-order guard — what CI runs:
npx tsc --noEmit && npm run lint && npm run typecheck:specs && npm test && npm run build && bash scripts/check-theme-link-order.sh
```

Nine test projects: `Core.Tests`, `Context.Tests`, `Host.Tests`, `MediaLibrary.Tests`, `Orchestration.Tests`, `Tts.Tests`, `Ads.Tests`, `Plugins.Tests`, and `Architecture.Tests` — the last enforces the architecture laws (dependency direction, Postgres/HttpClient confinement, contract immutability, the Host graduation tripwire) as ordinary red-green tests; the laws themselves are summarized front-and-center in [CONTRIBUTING.md](CONTRIBUTING.md). CI runs the PR tier on every pull request; the full suite, the on-air gate and the smoke test are run by the maintainer before a release.
...
GenWave's epic-by-epic history — from v1 broadcast playout through the truth gate and owner
announcements (v5.4), the self-tending library (v5.5), the plugin door and ads (v5.6), voice
and jingle packs (v5.7), sponsors and the guided spot (v5.8), and every patch since — lives in
[CHANGELOG.md](CHANGELOG.md), newest at the bottom.

## Roadmap

- **Deferred** — authored-file GC ([gh-#3](https://github.com/GenWave-Org/genwave/issues/3)), origin-side Access JWT validation ([gh-#75](https://github.com/GenWave-Org/genwave/issues/75)), migration-runner adoption ([gh-#12](https://github.com/GenWave-Org/genwave/issues/12)), a purge-tripwire override for legitimate whole-library replacement ([gh-#656](https://github.com/GenWave-Org/genwave/issues/656)), scan-side dedupe for directory symlinks inside `MediaRoot` ([gh-#650](https://github.com/GenWave-Org/genwave/issues/650)).
...
- **Upgrading an existing deployment:** `git pull && ./launch.sh --pinned` on an appliance/demo box, `./launch.sh` on the dev stack — both run `./migrate.sh` themselves (every `db/*-migration.sh`, idempotent, before anything restarts onto the new images). Call `./migrate.sh` by hand only on a box that doesn't use `launch.sh` at all; a raw `docker compose up` does **not** migrate. See [DEPLOYMENT.md](DEPLOYMENT.md).
- **A library row the scan can no longer verify is quarantined, never fed to the engine** (v5.4.0): rows discovered under a previous `MEDIA_DIR` root are marked unavailable after the scan's miss grace, and a push checks the file exists first — a moved library degrades to fewer tracks, not to silent safe-loops.
...
**One deliberate exception:** the module contract surface in [`src/GenWave.Abstractions/`](src/GenWave.Abstractions/) (published as the `GenWave.Abstractions` nuget package — its version tracks the release tag, additive across 5.x) is **MIT-licensed** — see [its LICENSE](src/GenWave.Abstractions/LICENSE) — so any module, open or commercial, can link the contracts freely. Everything else in this repository is AGPL-3.0-only.
