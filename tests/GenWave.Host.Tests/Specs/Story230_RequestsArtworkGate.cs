...
public static class FeatureRequestsArtworkGate
{
    ...
    public static class SadPathDisclosure
    {
        ...
        public static void TheEngineAndComposeHashesAreRePinnedAsThisEpoch()
        {
            ...
            // ComposeYamlSha256 re-pinned 2026-08-18 (PLAN T316, SPEC F136.2, STORY-343):
            // api's depends_on: kokoro gains `required: false` — the core `up` no longer
            // blocks/fails on an absent or not-yet-healthy kokoro (staged startup —
            // launch.sh's pinned flow pulls + brings up db/icecast/engine/api[/piper] first,
            // scoped `--no-deps` so kokoro can't ride along via depends_on membership; a
            // second, UNSCOPED pull + `up -d --remove-orphans` (no --no-recreate) then
            // converges kokoro/ollama/ollama-init plus every other profile-gated service).
            // Same T85/T93 epoch-break-and-re-pin ritual; a `depends_on`-only edit — no
            // service, wire, volume or healthcheck change.
            // ComposeYamlSha256 re-pinned 2026-09-06 (PLAN T412, SPEC F166.1/F166.2, STORY-395):
            // a new `voices` named volume plus the `voice-seed` one-shot init service — api
            // gains a read-write mount at `/voices` (F164.5) and a `required: false` depends_on
            // on voice-seed; kokoro gains a read-only mount at its own flat scan dir plus a
            // `required` (default) depends_on on voice-seed, so a failing seed fails the whole
            // `up` (fail-loud, same T85/T93 ritual). Comment-only follow-up same task: api's
            // mount notes it writes as root (no `USER` in its Dockerfile) into the volume's
