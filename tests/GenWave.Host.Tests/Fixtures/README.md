...
## `abstractions-5.7.0-surface.txt` — the Abstractions package surface pin (PLAN T453, STORY-431 AC6)

Generated by `Support/PublicSurface.cs`'s `PublicSurface.Of` from the PUBLISHED
`genwave.abstractions.5.7.0.nupkg`'s `lib/net10.0/GenWave.Abstractions.dll` (never from a branch
build): one provenance header line, then every exported type and public/protected member as one
line each, `LC_ALL=C sort`-ordered with no duplicates and no assembly identity (`Version=`,
`PublicKeyToken`).

`Specs/Story431_SponsorSettingsAndRelease.cs`'s `ScenarioAbstractionsUnchanged.ThePackageSurfaceDiffVs570IsEmpty`
diffs the surface of the build under test against this file and requires an empty diff (SPEC F176.4:
a release that never touched `src/GenWave.Abstractions` ships a byte-identical package surface).
