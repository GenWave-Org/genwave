...
## `abstractions-5.10.0-surface.txt` — the Abstractions package surface pin (PLAN T453/T530, STORY-431 AC6)

Generated by `Support/PublicSurface.cs`'s `PublicSurface.Of`: one provenance header line, then every
exported type and public/protected member as one line each, `LC_ALL=C sort`-ordered with no
duplicates and no assembly identity (`Version=`, `PublicKeyToken`).

Two provenances are permitted for the dll `PublicSurface.Of` runs against, and the header line
states which one applies:

- the **PUBLISHED** `genwave.abstractions.<version>.nupkg`'s `lib/net10.0/GenWave.Abstractions.dll`,
  when a package already exists for the version this fixture is named after; or
- the **release commit's own Release build** of `src/GenWave.Abstractions`, for the version being
  cut in the same PR that bumps the fixture — because the package only publishes once Dean tags that
  version (a later PLAN item), so no nupkg exists yet at bump time.

Never a branch build otherwise, and never a hand-edit either way.

`Specs/Story431_SponsorSettingsAndRelease.cs`'s `ScenarioAbstractionsUnchanged.ThePackageSurfaceDiffVs5100IsEmpty`
diffs the surface of the build under test against this file and requires an empty diff, so the package
surface stays byte-identical between regenerations. (SPEC F176.4 is where that check started — it
ruled v5.8.0 ship with "**Abstractions unchanged** … verified by package diff at the release cut";
this fixture generalises it to every release since.) **Regenerate ONLY by** running
`PublicSurface.Of` against one of the two permitted dlls above when a legitimate Abstractions bump
ships, renaming the file to that version in the same PR. Never hand-edit this file to tolerate a
diff.

⚠️ **And always diff the regenerated file against the previous baseline before committing it.** Under
the second provenance the fixture is derived from the same build the fact reads, so the fact cannot
fail at the commit that regenerates it — "never hand-edit" has no teeth there, because nobody needs
to hand-edit, they need only regenerate. So account for every line of that diff by hand: additions
must be exactly the contract change the PR intends, and **removals must be zero**. A removal is a
breaking change and needs a major bump, not a new baseline. Put the two counts in the PR body.
