...
public static class FeatureAdBedPickerStampsABedPerSpot
{
    ...
    public sealed class ScenarioTheRenderDucksTheBed
    {
        ...
        public void TheRenderedWaveformShowsABedAtTheDuckedLevelUnderTheVoice()
            => Assert.Fail("pending: T422 wire — AC2 (ffmpeg-provable waveform proof against a real rendered spot; T416 already emits the duck volume filter and passes BedDuckDb through unchanged — this fact needs a genuine rendered artifact, T422's own surface, not a filter-string pin)");

        [Fact]
        public void TheBedRunsUnduckedInTheTailAfterTheVoiceEnds()
            => Assert.Fail("pending: T422 wire — AC2 (ffmpeg-provable waveform proof; T416 does not change the duck/tail shape at all — F168 only adds a fade ON TOP of the existing tail, this fact needs a genuine rendered artifact, T422's own surface)");
    }
