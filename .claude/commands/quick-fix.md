...
3. **Consult skills selectively.** Only load a code-quality skill if the
   ...
   - `csharp-best-practices`: consult if the fix involves C# nullability,
     async, error shape, or type modeling.
   - `typescript-best-practices`: same territory in the TS/UI code.
4. **Apply the edit.** Use `Edit`, not `Write`. Match surrounding style.
5. **Verify.** Run the project's build and tests if they're fast. If
   they're slow or absent, run the narrowest check that proves the fix
   (e.g. `dotnet build` on the affected project, `tsc --noEmit` on the
