...
- DB schema → `postgres-dba`.
- Module/class boundaries → `solid-principles`, `design-principles`.
- Pattern choice → `gof-patterns`.
- C# conventions → `csharp-best-practices`; API/service structure
  (endpoints, hosted services, options) → `aspnetcore-patterns`.
- Auth/trust boundaries, API security posture → `security-api`.
- Containers, compose topology, volumes/NFS → `docker-linux-ops`.
- TS conventions (Admin UI work) → `typescript-best-practices`.
