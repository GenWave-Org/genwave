...
description: >-
  ...
  missing. Generates spec files in nested blocks — Feature > Scenario >
  Specification — where each Scenario arranges all of its data once,
...
- **Use xUnit** if the feature under spec is C#/.NET: a `.sln` or
...
Specification**. In TypeScript the blocks are nested `describe`s; in
xUnit the Feature is a top-level class, each Scenario is a nested class
(arrange in the constructor or an `IClassFixture`), and each
Specification is a `[Fact]` — see `templates/Feature.xunit.cs`.
...
1. **Feature** is the outermost block (`describe` / top-level class),
   titled `Feature: …` with a short human-readable name — **no story id
   in the title**. One feature per file, sourced from a user story. The
   file name is a readable slug of the feature, also without a story-id
   prefix (kebab-case `atomic-create-user-order-entitlements.spec.ts`
   for TS, PascalCase `AtomicCreateUserOrderEntitlements.cs` for C#).
   Put the story id in a single comment at the top of the file
   (e.g. `// STORY-007 — Atomic create of user, order, entitlements`) —
   that comment is the only place the id appears.
2. **Scenario** is a nested block (`describe` / nested class), titled
   `Scenario: …`. Every Scenario arranges **all** of its required data
   once — `beforeAll` in TS (use `beforeEach` only if a spec mutates
   shared state), the constructor or an `IClassFixture` in xUnit. No
   arrange logic inside the spec bodies.
3. **Specification** is one `it` / one `[Fact]`. **Exactly one
   assertion each.** No multiple `expect`s/`Assert`s, no looped
   assertions. Need another assertion → another spec.
...
5. **Sad path is segregated.** Error, validation, and edge-case specs
   ...
   happy-path specs.
6. **Fully typed in the project's language** — no `any` in TS, no
   `dynamic`/`object` bags or `!` in C# (outside deliberate null-path
   specs). Imports/usings come from the detected runner only.
7. **At least one Scenario per Feature exercises the deployed entry
   point.** Drive the production surface — the ASP.NET Core endpoint via
   `WebApplicationFactory<Program>`, the exported handler,
   `default.fetch`, the Next route file — not an internal
