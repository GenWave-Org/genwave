...
name: gof-patterns
description: >-
  Gang of Four design patterns reference — language-agnostic; applies
  equally to C# and TypeScript work (examples are written in TypeScript).
  Use when choosing or implementing a design pattern, refactoring toward a
  known pattern, reviewing code for pattern misuse, or answering "which
  pattern fits here" questions. Covers all 23 creational, structural, and
  behavioral patterns with examples and guidance on when (and when not) to
  use each.
---
