...
-- Rule 1/2: singular table, `id serial primary key`, natural key is UNIQUE.
...
CREATE INDEX ix_orders_customer_id ON purchase_order (customer_id);
CREATE INDEX ix_orders_status      ON purchase_order (status);

CREATE TABLE order_item (
  id          serial primary key,
  order_id    int not null references purchase_order   (id) on delete cascade,  -- child dies with parent
  product_id  int not null references product (id) on delete restrict, -- can't delete a sold product
...
CREATE INDEX ix_order_items_order_id ON order_item (order_id);
